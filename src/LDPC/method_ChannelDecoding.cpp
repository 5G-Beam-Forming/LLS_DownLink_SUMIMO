...
void NR_ChannelCoding::Rx_RateMatching (NR_CODEC *nr_codec)	// additional coding  (NR_CODEC을 input으로 받음)
{
	...
	//		}
	//		// rx_e에서 rx_d를 한번만 읽어 올때는 아래 else if와 else는 주석처리
	...
	*/
	...
	/* 수정 후 code */		// additional coding
	// 위 code를 //from ~ //to 까지 수정
	//from
	for (r=0; r<C; r++){		
		j = 0;					
		k = 0;					
...
void NR_ChannelCoding::Rx_LDPCDecoding (NR_CODEC *nr_codec) 
{
	...
	for (r=0; r<C; r++){
		...
		for(k=0; k<N_H; k++){
			if(rx_d[r][k] != 0){
				rx_d[r][k] = -rx_d[r][k] ; // demodulator가 LLR 을 log p(1)/p(0)로 정의함
			}
		}
	}
...
// 기존 Rx_CRC_Detachment code를 아래의 code로 교체 // additional coding // additional coding // additional coding
